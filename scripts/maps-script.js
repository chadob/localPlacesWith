...
var mapFunctions = {
  ...
  filterByQuery: function() {
    ...
    var searchProps = ["zip", "state", "city"];
    if (query.length > 0) {
      return mapFunctions.allMarkers.partition(mapSearchFunctions.checkQuery(searchProps, query))
    } else {
      return [mapFunctions.allMarkers, []]
    }
  },
  filterByCat: function(catArray) {
    if (catArray.length > 0) {
      ...
      return [mapFunctions.currentMarkers, mapFunctions.hiddenMarkers]
    }
  },
  //searches categories selected against all markers
  filterAllMarkersAndListItems: function(catArray, color) {
    var joinedArr;
    joinedArr = mapFunctions.filterByQuery();
    mapFunctions.currentMarkers = joinedArr[0];
    mapFunctions.hiddenMarkers = joinedArr[1];
    if (color) {
      joinedArr = mapFunctions.filterByCat(catArray);
      mapFunctions.currentMarkers = joinedArr[0];
      mapFunctions.hiddenMarkers = joinedArr[1];
    }
  ...
  setUpCard: function(location, section, id, adjustedSection) {
    ...
    '<p class="gmap-card-title">' + location.venueName + '</p>' +
